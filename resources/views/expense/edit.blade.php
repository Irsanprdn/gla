        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="text" name="expense_name" value="{{ $expense->expense_name }}" id="expense_name" class="form-control" required>
                </div>

                <div class="form-group">
                    <label for="amount">Amount</label>
                    <input type="number" name="amount" value="{{ $expense->amount }}" id="amount" class="form-control" required step="0.01" oninput="formatAmount()">
                    <small id="formattedAmount" class="form-text text-muted"></small>
                </div>

                <div class="form-group">
                    <label for="date">Date</label>
                    <input type="date" name="date" id="date" value="{{ date('Y-m-d', strtotime($expense->date)) }}" class="form-control" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="bank_account_id" id="bank_account_id" class="form-control" required>
                        ...
                        <option value="{{ $bank->id }}" {{ $expense->bank_account_id == $bank->id ? 'selected' : ''}}>{{ $bank->nama_bank }} - {{ $bank->account_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
